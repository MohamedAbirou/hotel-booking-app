...
test("should allow the user to register", async ({ page }) => {
  ...
  const testFirstName = `${Math.floor(Math.random() * 90000) + 10000}`;
  const testLastName = `${Math.floor(Math.random() * 90000) + 10000}`;
  ...
  await page.locator("[name=confirmPassword]").fill("qwer1234");
